...
func TestHandler_shortenURLJSON(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expectedBody:      gin.H{"error": "error from GetRowData()"},
			shortenStringCall: 0,
			expectedJSONCall:  1,
		},
		{
			...
			serviceReturn:     shortenURLReturn{result: "", error: nil},
			expectedCode:      http.StatusBadRequest,
			expectedBody:      gin.H{"error": "invalid character 'h' looking for beginning of value"},
			shortenStringCall: 0,
			expectedJSONCall:  1,
		},
		{
			...
			expectedCode:      http.StatusBadRequest,
			expectedBody:      gin.H{"error": "error from shorten service"},
			shortenStringCall: 1,
			expectedJSONCall:  1,
		},
	}
